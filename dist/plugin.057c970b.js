...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"plugin/chalkboard/plugin.js":[function(require,module,exports) {
/*****************************************************************
...
window.RevealChalkboard = window.RevealChalkboard || {
  ...
  configure: function (_configure) {
    function configure(_x) {
      return _configure.apply(this, arguments);
    }

    configure.toString = function () {
      return _configure.toString();
    };
  ...
  toggleNotesCanvas: function (_toggleNotesCanvas) {
    function toggleNotesCanvas() {
      return _toggleNotesCanvas.apply(this, arguments);
    }

    toggleNotesCanvas.toString = function () {
      return _toggleNotesCanvas.toString();
    };

    return toggleNotesCanvas;
  }(function () {
    toggleNotesCanvas();
  }),
  toggleChalkboard: function (_toggleChalkboard) {
    function toggleChalkboard() {
      return _toggleChalkboard.apply(this, arguments);
    }

    toggleChalkboard.toString = function () {
      return _toggleChalkboard.toString();
    };

    return toggleChalkboard;
  }(function () {
    toggleChalkboard();
  }),
  colorIndex: function (_colorIndex) {
    function colorIndex() {
      return _colorIndex.apply(this, arguments);
    }

    colorIndex.toString = function () {
      return _colorIndex.toString();
    };
  ...
  colorNext: function (_colorNext) {
    function colorNext() {
      return _colorNext.apply(this, arguments);
    }

    colorNext.toString = function () {
      return _colorNext.toString();
    };
  ...
  colorPrev: function (_colorPrev) {
    function colorPrev() {
      return _colorPrev.apply(this, arguments);
    }

    colorPrev.toString = function () {
      return _colorPrev.toString();
    };
  ...
  resetAll: function (_resetAll) {
    function resetAll() {
      return _resetAll.apply(this, arguments);
    }

    resetAll.toString = function () {
      return _resetAll.toString();
    };
  ...
  updateStorage: function (_updateStorage) {
    function updateStorage() {
      return _updateStorage.apply(this, arguments);
    }

    updateStorage.toString = function () {
      return _updateStorage.toString();
    };
...
function scriptPath() {
  ...
  var path = src === undefined ? "" : src.slice(0, src.lastIndexOf("/") + 1); //console.log("Path: " + path);
...
var initChalkboard = function initChalkboard(Reveal) {
  //console.warn(path);
  ...
  function whenReady(callback) {
    ...
    if (!document.querySelector('section[data-markdown]:not([data-markdown-parsed])') && !document.querySelector('code[data-line-numbers*="|"]')) {
      callback();
    } else {
  ...
  function createPalette(colors, length) {
    ...
    var list = document.createElement('ul'); // color pickers
  ...
  function switchBoard(boardIdx) {
    selectBoard(boardIdx, true); // broadcast
  ...
  function getSlideData(indices, id) {
    ...
    var page = Number(Reveal.getCurrentSlide().getAttribute('data-pdf-page-number')); //console.log( indices, Reveal.getCurrentSlide() );
  ...
  var printMode = /print-pdf/gi.test(window.location.search); //console.log("createPrintout" + printMode)
  ...
  function addPageNumbers() {
    ...
    for (var i = 0; i < slides.length; i++) {
      slides[i].setAttribute('data-pdf-page-number', page.toString()); // add number of fragments without fragment indices
  ...
  function createPrintout() {
    ...
    patImg.onload = function () {
      var slides = Reveal.getSlides(); //console.log(slides);
      ...
      for (var i = storage[1].data.length - 1; i >= 0; i--) {
        ...
        var drawings = createDrawings(slideData, patImg); //console.log("Page:", storage[ 1 ].data[ i ].page );
  ...
  function getCanvas(template, container, board) {
    var idx = container.findIndex(function (element) {
  ...
  function createDrawings(slideData, patImg) {
    ...
    mode = 1;
    board = 0; //		console.log( 'Create printout(s) for slide ', slideData );
    ...
    for (var j = 0; j < slideData.events.length; j++) {
      switch (slideData.events[j].type) {
        case 'draw':
          draw[1](getCanvas(template, drawings, board).getContext('2d'), xOffset + slideData.events[j].x1 * scale, yOffset + slideData.events[j].y1 * scale, xOffset + slideData.events[j].x2 * scale, yOffset + slideData.events[j].y2 * scale, yOffset + slideData.events[j].color);
          break;

        case 'erase':
          eraseWithSponge(getCanvas(template, drawings, board).getContext('2d'), xOffset + slideData.events[j].x * scale, yOffset + slideData.events[j].y * scale);
          break;
  ...
  function addDrawings(slide, drawings) {
    ...
    for (var i = 0; i < drawings.length; i++) {
      ...
      newPDFPage.append(drawings[i].canvas); //console.log("Add drawing", newPDFPage);
  ...
  function processQueue() {
    ...
    var message = eventQueue.shift(); // synchronize time with seminar host

    slideStart = Date.now() - message.content.timestamp; // set status
  ...
  function startPlayback(timestamp, finalMode) {
    ...
    for (var id = 0; id < 2; id++) {
      clearCanvas(id);
      var slideData = getSlideData(slideIndices, id); //console.log( timestamp +" / " + JSON.stringify(slideData));
    ...
    if (mode == 1) showChalkboard(); //console.log("playback (ok)");
  }
  ...
  function erasePoint(x, y) {
    ...
    var yOffset = drawingCanvas[mode].yOffset; // move sponge image
    ...
    if (x * scale + xOffset > 0 && y * scale + yOffset > 0 && x * scale + xOffset < drawingCanvas[mode].width && y * scale + yOffset < drawingCanvas[mode].height) {
      eraseWithSponge(ctx, x * scale + xOffset, y * scale + yOffset);
    }
  }

  function stopErasing() {
    erasing = false; // hide sponge
  ...
  function drawSegment(fromX, fromY, toX, toY, colorIdx) {
    ...
    if (fromX * scale + xOffset > 0 && fromY * scale + yOffset > 0 && fromX * scale + xOffset < drawingCanvas[mode].width && fromY * scale + yOffset < drawingCanvas[mode].height && toX * scale + xOffset > 0 && toY * scale + yOffset > 0 && toX * scale + xOffset < drawingCanvas[mode].width && toY * scale + yOffset < drawingCanvas[mode].height) {
  ...
  function setupCanvasEvents(canvas) {
    ...
    canvas.addEventListener('touchstart', function (evt) {
      evt.preventDefault(); //console.log("Touch start");
    ...
    canvas.addEventListener('touchmove', function (evt) {
      evt.preventDefault(); //console.log("Touch move");
      ...
      if (drawing || erasing) {
        ...
        if (drawing) {
          drawSegment((lastX - xOffset) / scale, (lastY - yOffset) / scale, (mouseX - xOffset) / scale, (mouseY - yOffset) / scale, color[mode]); // broadcast
          ...
          erasePoint((mouseX - xOffset) / scale, (mouseY - yOffset) / scale); // broadcast
    ...
    canvas.addEventListener('touchend', function (evt) {
      evt.preventDefault();
      clearTimeout(touchTimeout);
      touchTimeout = null; // hide sponge image
    ...
    canvas.addEventListener('mousedown', function (evt) {
      ...
      if (!readOnly && evt.target.getAttribute('data-chalkboard') == mode) {
        ...
        if (evt.button == 2 || evt.button == 1) {
          startErasing((mouseX - xOffset) / scale, (mouseY - yOffset) / scale); // broadcast
    ...
    canvas.addEventListener('mousemove', function (evt) {
      evt.preventDefault(); //console.log("Mouse move");
  ...
  function resize() {
    ...
    for (var id = 0; id < 2; id++) {
      ...
      drawingCanvas[id].scale = Math.min(drawingCanvas[id].width / storage[id].width, drawingCanvas[id].height / storage[id].height);
      drawingCanvas[id].xOffset = (drawingCanvas[id].width - storage[id].width * drawingCanvas[id].scale) / 2;
      drawingCanvas[id].yOffset = (drawingCanvas[id].height - storage[id].height * drawingCanvas[id].scale) / 2; //console.log( drawingCanvas[id].scale + "/" + drawingCanvas[id].xOffset + "/" +drawingCanvas[id].yOffset );
  ...
  Reveal.addEventListener('fragmentshown', function (evt) {
    ...
    if (!printMode) {
      ...
      if (Reveal.isAutoSliding()) {
        ...
        startPlayback(getSlideDuration(), 0); //				closeChalkboard();
      }
    }
  });
  ...
  Reveal.addEventListener('autoslidepaused', function (evt) {
    ...
    document.dispatchEvent(new CustomEvent('stopplayback')); // advance to end of slide
    //		closeChalkboard();
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","plugin/chalkboard/plugin.js"], null)
//# sourceMappingURL=/plugin.057c970b.js.map